...
tasks = {
    ...
    'TiDB.tikv.TiKV_GC_can_not_work': {
        'warning_level': 'critical',
        'pql': 'increase(tikv_gcworker_gc_tasks_vec{task="gc",instance=~"%s.*"}[2d]) < 1 and increase('
               'tikv_gc_compaction_filter_perform{instance=~"%s.*"}[2d]) < 1' % (self_ip, self_ip),
        'is_value': False
    },
    ...
    'TiDB.tikv.TiKV_coprocessor_request_error': {
        ...
        'pql': 'increase(tikv_coprocessor_request_error{reason!="lock", instance=~"%s.*"}[10m])' % self_ip,
        'is_value': True
    },
    ...
    'TiDB.tikv.TiKV_coprocessor_pending_request': {
        'warning_level': 'warning',
        'pql': 'delta('
               'tikv_coprocessor_pending_request{instance=~"%s.*"}[10m])' % self_ip,
        'is_value': True
    },
